...
class modelUsers {
	...
	*/
	...
	public function _getUsers2($app,$prefix)
	{
	
		$query  = 'SELECT user.usuario_id as id, concat(user.nombres," ", user.apellidos) as name, role.name as rol FROM usuario user,'.'`'.$prefix.'admin_roles` role where user.tipo=role.rol_id';
        $list 	= $app['dbs']['mysql_silex']->fetchAll($query);
    	return $list;
	}
...
/**
 ...
 public function _deleteUser2($app,$id,$prefix,$logError)
 {
   try {

     $del=$app['dbs']['mysql_silex']->delete('usuario' , array('usuario_id' => $id));
     $logError->addLine('Admin Users',"{admin_users}","Eliminacion de usuario ".$id, $_SESSION["uid"]);
   }
   catch(Exception $exception) 
   { 
...
/**
	...
	* @return object
	*/
	public function _getTableEditUser2($app,$prefix,$iduser)
	{
		$optionmenu 			= new stdClass();
		$optionmenu->title_form = "Editar Usuario";
      
        $query 			= 'SELECT nombres as name, apellidos lastname,  usuario FROM usuario WHERE usuario_id ='.$iduser;
		$params 		= $app['dbs']['mysql_silex']->fetchAssoc($query);
	
        $roles=$this->_getRoles($app,$prefix);

		$html = '<form id="form-params" class="row-fluid ">';
		$html.= '<input type="hidden" name="task" value="edit-user2">';
		$html.= "<input type='hidden' name='id' value='$iduser'>";
		$html.= '<fieldset>';
		$html.= '<legend></legend>';      
        $html.= '<div id="facebook-admin-box" class="facebook-admin-box input-prepend ">';
		...
		$html.= '</div>';
		$html.= '<div id="facebook-admin-box" class="facebook-admin-box input-prepend ">';
		$html.= '<span class="add-on facebook-admin-title span3"><strong>Apellido:</strong></span>';
		$html.= '<input type="text" class="span4 facebook-admin-input" name="lastname" value="'.$params['lastname'].'" />';
		$html.= '<a class="info-param-tooltip" data-content="Nombre" title=""  href="#" data-original-title="¿Cómo configurar?"><i class="icon-info-sign"></i></a>';
		$html.= '</div>';
		$html.= '<div id="facebook-admin-box" class="facebook-admin-box input-prepend ">';
		$html.= '<span class="add-on facebook-admin-title span3"><strong>Usuario:</strong></span>';
		$html.= '<input type="text" class="span4 facebook-admin-input" name="user" value="'.$params['usuario'].'" />';
		$html.= '<a class="info-param-tooltip" data-content="Nombre" title=""  href="#" data-original-title="¿Cómo configurar?"><i class="icon-info-sign"></i></a>';
		$html.= '</div>';
		$html.= '<div id="facebook-admin-box" class="facebook-admin-box input-prepend ">';
		$html.= '<span class="add-on facebook-admin-title span3"><strong>Password:</strong></span>';
