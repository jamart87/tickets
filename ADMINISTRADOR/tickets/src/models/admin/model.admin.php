...
#### SETTING COOKIES PARAMETERS WITH SSL####
...
';
	...
	*/
	public function _getMenuAdministrator($app,$prefix)
	{
		...
		$menu.= '<span class="icon-bar"></span>';
		$menu.= '</a>';
		$menu.= '<a class="brand" href="#" onclick="javascript:window.location.href=\'../index.php/?op=administrator\'" >KLIKTICKET</a>';
	...
	* @return object
	*/
	public function _getTableParamsEventos($app, $idmenu,$prefix)
	{
		$optionmenu 	= new stdClass();
		...
		foreach ($content as $key => $value) {
            ...
            switch ($file_name_fb_samall) {
            	...
            	case 'Usuario':
            	    ...
            	    $count=0; 
            	    $query 		= 'SELECT * FROM `usuario` ORDER BY nombres, apellidos DESC';
				    $u 	        = $app['dbs']['mysql_silex']->fetchAll($query);
				    $select="<select name='usuario_id' id='user'>";
			    	$select .= "<option value=-1>Seleccione un usuario</option>";
			    	foreach ($u as $key) {
			          if($count==0)
			           $select .= "<option value=".$key["usuario_id"]." selected>".$key["nombres"]." ".$key["apellidos"]."</option>";
			    	  else
			    	    $select .= "<option value=".$key["usuario_id"].">".$key["nombres"]." ".$key["apellidos"]."</option>";	
			    	 $count+=1;
			    	}
				...
				case 'Evento':
            	    ...
            	    $count=0; 
            	    $query 		= 'SELECT * FROM `evento` ORDER BY nombre DESC';
				    $u 	        = $app['dbs']['mysql_silex']->fetchAll($query);
				    $select="<select name='evento_id' id='user'>";
			    	$select .= "<option value=-1>Seleccione un usuario</option>";
			    	foreach ($u as $key) {
			          if($count==0)
			           $select .= "<option value=".$key["evento_id"]." selected>".$key["nombre"]."</option>";
			    	  else
			    	    $select .= "<option value=".$key["evento_id"].">".$key["nombre"]."</option>";	
			    	 $count+=1;
			    	}
			    	$html.= '<div id="facebook-admin-box-" class="facebook-admin-box input-prepend ">';
					$html.= '<span class="add-on facebook-admin-title span3"><strong>'.$value['label'].':</strong> <strong class="small-view" >'.$file_name_fb_samall.':</strong> </span>';
	...
	*/
	public function _saveAdminForm( $app, $dataForm, $logError,$prefix)
	{
		...
		if ($id_conf == 11) {
       		$query 			= 'SELECT * FROM detalle_usuario WHERE usuario_usuario_id = '.(int)$dataForm[1]['value']. ' AND evento_evento_id ='.(int)$dataForm[2]['value'];
		    $check 		    = $app['dbs']['mysql_silex']->fetchAll($query);
            
            if (count($check) == 0) {
       		$query 			= 'INSERT INTO detalle_usuario (usuario_'.$dataForm[1]['name'].', evento_'.$dataForm[2]['name'].' ) ';
			$query 			.= 'VALUES( '.$dataForm[1]['value'].', '.trim($dataForm[2]['value']).' )';
	...
	* @return bool
	*/
	public function _saveLoginBackground( $app, $imgName, $logError,$prefix, $models )
	{
		@session_start();
...
*/
	...
	public function _saveAdminUsers( $app, $dataForm, $logError,$prefix,$models)
	{
       ...
       /*Validacion de los campos ingresados para el registro de los usuarios*/
		...
		try {
			...
			#guardar user app
			$query 			= "INSERT INTO usuario (nombres, apellidos, direccion, telefono, usuario, clave, tipo) ";
			$query 			.= 'VALUES( "'.$user['fname'].'","'.$user['lname'].'","'.$user['address'].'","'.$user['phone'].'","'.$user['uname'].'","'.$models->encryptAdmin($user['passuser']).'",'.$user['roluser'].')';
			$app['dbs']['mysql_silex']->executeQuery($query);
			} 
			return TRUE;
	...
	*/
	...
	public function _saveEditUsers2( $app, $dataForm, $logError,$prefix,$models)
	{ 
       ...
       /*Validacion de los campos ingresados para el registro de los usuarios*/
        ...
        try {
            $user['password']=$models->encryptAdmin($user['password']);
			#guardar registro en la tabla admin_users
            $update=$app['dbs']['mysql_silex']->update("usuario", array('nombres' => $user['name'],'apellidos' => $user['lastname'],'clave' => $user['password'], 'usuario'=> $user['user']), array('usuario_id' => $user['id']));
