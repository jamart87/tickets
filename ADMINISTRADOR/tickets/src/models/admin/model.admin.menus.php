...
/**
	...
	* @return boolean
	*/
 public function _deleteEvento($app,$id,$prefix,$logError)
 {
   try {
   	 $del=$app['dbs']['mysql_silex']->delete('detalle_usuario' , array('detalle_usuario_id' => $id));
     $logError->addLine('Admin Items',"{admin_params_conf}","Evento Eliminado ".$id, $_SESSION["uid"]);
...
/**
	...
	* @return object
	*/
	public function _getTableEditEventos($app,$prefix,$idmenu)
	{
		$optionmenu 			= new stdClass();
		$optionmenu->title_form = "Editar Item";
...
/**
	...
	* @return object
	*/
function _getEventosTable($app,$prefix)
{

   $query 	= 'SELECT A.detalle_usuario_id as id, CONCAT(B.nombres, " ", B.apellidos) as user, C.nombre FROM detalle_usuario AS A
				INNER JOIN usuario AS B ON A.usuario_usuario_id = B.usuario_id
				INNER JOIN evento  AS C ON C.evento_id = A.evento_evento_id';
   $params 	= $app['dbs']['mysql_silex']->fetchAll($query);
   $html='';
   $html.='<div id="tabla" class="table_menus">'; 
   $html.='<table class="table table-hover table-striped">';
   ...
   foreach ($params as $key => $value) {
     ...
     $html.='<td class="text-center"><span class="btn btn-danger btn-small" id="'.$value['id'].'"  onClick="deleteEvento(this.id)"  title="Eliminar"><i class="icon-remove"></i></span></td>';                          
     $html.='</tr>';
