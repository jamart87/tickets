...
$app->match('/inicio', function () use ($app, $admin,$models,$prefix) {
   ...
   #Opciones 
$app->match('/option-{id}', function ($id) use ($app, $admin,$language,$prefix,$templates,$install, $modelmenus) {
               $url 	= str_replace('/index.php', '', $_SERVER['SCRIPT_NAME'] );
		      $redirect = $admin->_validateSessionActive();
				if ($redirect->redirect ){
					return $app->redirect( $url.$redirect->url);	
					...
					$menu 	= $admin->_getMenuAdministrator($app,$prefix);
						switch ($id) {
							...
							break;
							case 11: #install template 	 	
							$form_table = $admin->_getTableParamsEventos($app,$id,$prefix); 	 	
							$template='params-admin.twig'; 	 	
							$params = array( 'TITLE_PAGE' => "Administrator", "MENU" => $menu, "FORM" => $form_table->form, "TITLE_FORM" => $form_table->title_form ); 	 	
							break; 	 	
							case 10:#admin template 	 	
							$template='params-admin.twig'; 	 	
							$menu = $admin->_getMenuAdministrator($app,$prefix); 	 	
							$list = $modelmenus->_getEventosTable($app,$prefix);
							$params = array( 'TITLE_PAGE' => "Eventos por Usuario", "MENU" => $menu, "FORM" => $list, "TITLE_FORM" => "Eventos por Usuario");
							break;
							default:
	...
	$app->match('/adminform', function () use ($app, $admin, $logError, $prefix,$models,$language, $modelmenus) {
