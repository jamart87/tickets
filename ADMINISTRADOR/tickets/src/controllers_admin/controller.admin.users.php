...
$app->match('/Users', function () use ($app, $admin,$prefix,$modelusers) {     
          ...
          $url 	= str_replace('/admin/index.php', '', $_SERVER['SCRIPT_NAME'] ).'/admin';
				...
				if ($redirect->redirect ){
					...
					$users2 = $modelusers->_getUsers2($app,$prefix);

					$table_users= users($users, $users2);
...
$app->match('/Edit_User2-{id}', function ($id) use ($app, $admin,$prefix,$modelusers) {     
		    ...
		    $url 	= str_replace('/admin/index.php', '', $_SERVER['SCRIPT_NAME'] ).'/admin';
				...
				if ($redirect->redirect ){
					...
					$menu 	= $admin->_getMenuAdministrator($app,$prefix);
					$form_table = $modelusers->_getTableEditUser2($app,$prefix,$id);
...
$app->match('/delete_user2', function () use ($app, $admin,$prefix,$modelusers,$logError) {     
        ...
        $id=$_POST['id'];   
				...
				if ($redirect->redirect ){
					...
					$menu 	= $admin->_getMenuAdministrator($app,$prefix);
				   	$delete = $modelusers->_deleteUser2($app,$id,$prefix,$logError);
					 if($delete!=NULL)
                	   return 'Usuario Eliminado Correctamente';
              		else
                	  return 'Error al Eliminar el Usuario';
